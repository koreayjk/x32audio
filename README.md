# X32 교회 음향 (Phase 1)

Behringer **X32 / M32** 디지털 믹서를 위한 교회 음향 자동화 데스크탑 앱입니다.
OSC(UDP) 로 콘솔과 통신하며, 예배 진행에 맞춘 Scene 전환과 피드백(하울링) 경고를
한국어 UI 로 제공합니다.

> **기술 스택**: Electron + Node.js · **통신**: OSC over UDP (X32 기본 포트 `10023`) ·
> **OSC 라이브러리**: [`node-osc`](https://www.npmjs.com/package/node-osc) ·
> **배포**: Windows / macOS (electron-builder)

---

## Phase 1 기능

1. **연결** — X32 IP 입력 후 연결하고 콘솔 정보(모델·펌웨어)를 확인합니다.
2. **채널 상태 읽기** — 채널별 이름 / 레벨(dB) / 음소거 / EQ 상태를 읽어옵니다.
   ("EQ 보기" 로 4밴드 EQ(주파수·게인·Q) 상세도 확인.)
3. **피드백 감지** — RTA 스펙트럼을 모니터링해 지속되는 좁은 피크(하울링)를
   감지하고 추정 주파수와 함께 경고합니다.
4. **Scene 템플릿** — 교회용 기본 장면(설교 / 찬양팀 / 기도·묵상 / 광고 /
   전체 음소거 / 기본 셋업)을 한 번에 적용합니다.
5. **한국어 UI** + 첫 실행 단계별 사용 가이드.

## 추가 기능

6. **예배 순서 큐(Cue List)** — 장면을 순서대로 묶어두고 **스페이스바**로
   다음 장면을 차례차례 적용합니다. (순서는 자동 저장)
7. **사용자 정의 Scene 저장/불러오기** — 현재 콘솔의 채널 상태를 이름 붙여
   저장했다가(예: "주일예배 1부") 다음에 그대로 호출합니다.
8. **자동 피드백 억제** — 감지된 주파수를 31밴드 GEQ의 가장 가까운 밴드로
   매핑해 단계적으로 감쇠(노치)합니다. 안전 한도가 있고, 끄면 0dB로 복원됩니다.
   (콘솔 한 FX 슬롯에 GEQ를 메인 L/R 인서트로 올려두고 슬롯 번호를 지정하세요.)
9. **인물별 마이크 프리셋** — 채널의 EQ·로우컷·트림을 사람 이름으로 저장했다가
   (예: "담임목사님") 그날 그 사람이 쓰는 채널에 그대로 적용합니다.
   (채널의 "EQ 보기" 창에서 저장/적용)
10. **실시간 양방향 제어** — 콘솔에서 바꾸면 앱에 즉시 반영(실시간 토글)되고,
    앱의 채널 표에서 페이더를 끌거나 ON/MUTE를 눌러 **콘솔을 직접 제어**합니다.
11. **되돌리기(Undo)** — 장면/큐/사용자 Scene 적용 직전 상태로 한 번에 복구.
12. **백업/복원** — 사용자 정의 Scene·큐·프리셋(+콘솔 상태)을 JSON 파일로
    내보내고 다시 불러옵니다.
13. **봉사자 쉬운 모드** — 큰 Scene 버튼 위주로 UI를 단순화.
14. **다국어** — 한국어 / English 전환.
15. **태블릿/폰 원격 조작** — 내장 서버를 켜면 같은 네트워크의 기기 브라우저에서
    큰 버튼으로 **Scene 전환 · 예배 순서 큐(이전/다음) · 전체 음소거**를 제어합니다.
    (별도 설치 불필요, 외부 패키지 없이 동작)
16. **3개 출력 분리 관리** — Main LR(본당) · Bus 1(유튜브/방송) · Bus 2(찬양팀 모니터)를
    각각 다른 믹스로 운용합니다.
    - **예배 시작** 버튼 하나로 세 출력을 동시에 프리셋 적용
    - **설교 모드** — 설교 시 방송 버스에서만 악기 레벨을 자동으로 낮춤(본당/모니터는 유지)
    - **방송 자동 레벨** — 방송 버스를 **LUFS −14**(유튜브 기준)에 맞춰 자동 보정
17. **믹서 선택 화면** — 첫 실행 시 믹서 종류를 고릅니다.
    - **디지털**: Behringer X32/M32(지원) · 기타(추후 지원 예정)
    - **아날로그 믹서 가이드** — 컴퓨터 마이크로 현장 소리를 실시간 분석해,
      어떤 노브(GAIN/HIGH/MID/LOW)를 어느 방향으로 돌려야 하는지 **믹서 그림/사진
      위 해당 노브에** 링+화살표로 시각 안내합니다. (입력 과다·하울링·음색 불균형 자동 판정)
    - **내 믹서 사진 매핑** — 기본 믹서 그림 대신 **내 믹서 사진을 업로드**하고
      GAIN·HIGH·MID·LOW 위치를 한 번 클릭해 지정하면, 실제 내 믹서 사진 위에서
      안내받습니다. (위치는 저장되어 다음에도 유지)

> 📄 프로젝트 소개용 **랜딩페이지**는 `landing/index.html` 에 있습니다.

---

## 설치 및 실행

```bash
npm install        # 의존성 설치 (electron, node-osc)
npm start          # 앱 실행
npm run dev        # 개발자 도구를 띄운 채 실행
```

### 테스트

OSC 통신·변환·피드백 감지·Scene 로직은 가짜 X32(`test/mock-x32.js`)를 이용해
실제 UDP 루프백으로 검증합니다. (Electron 불필요)

```bash
npm test
```

### 배포 패키지 빌드

```bash
npm run dist:win   # Windows (NSIS 설치 파일)
npm run dist:mac   # macOS (DMG)
```

> 아이콘을 넣으려면 `build/icon.ico`(Windows), `build/icon.icns`(macOS) 를 추가하세요.
> 없어도 기본 아이콘으로 빌드됩니다.

---

## 사용 방법

> 💡 **처음 실행하면 단계별 사용 가이드가 자동으로 뜹니다.** 화면을 하나씩
> 밝게 강조하며 "어디를 누르면 무엇이 되는지" 안내합니다. 언제든 상단 우측의
> **[❓ 사용 가이드]** 버튼으로 다시 볼 수 있습니다.

1. 음향 PC 와 X32 를 같은 네트워크(유선 권장)에 연결합니다.
2. 앱 상단에 X32 **IP 주소**를 입력하고 **연결**을 누릅니다.
   (X32 의 IP 는 콘솔 `Setup → Network` 에서 확인)
3. **상태 읽기** 로 현재 채널 상태를 불러옵니다.
4. 예배 순서에 맞춰 **Scene 템플릿**을 눌러 적용합니다.
5. **피드백 감지 시작** 으로 하울링 모니터링을 켭니다.

### 피드백 감지 정확도 높이기

피드백 감지는 X32 의 **RTA(`/meters/15`)** 데이터를 사용합니다.
콘솔에서 RTA 소스를 **메인 L/R(스피커로 나가는 신호)** 로 지정하면
실제 하울링을 가장 잘 잡아냅니다. 우측 **민감도** 슬라이더로 감도를 조절하세요.

> 감지된 주파수는 "추정값"입니다. 해당 주파수를 GEQ/PEQ 에서 살짝 감쇠하면
> 하울링을 줄일 수 있습니다. 긴급 시 **전체 음소거** Scene 을 사용하세요.

---

## 교회용 기본 채널 배치

`src/main/scenes.js` 의 `DEFAULT_CHANNEL_MAP` 에서 우리 교회 콘솔에 맞게
채널 번호·이름·역할(`speech`/`vocal`/`inst`/`playback`)을 수정하세요.
역할에 따라 Scene 동작과 로우컷(HPF) 적용이 달라집니다.

| 채널 | 이름 | 역할 |
|------|------|------|
| 1 | 설교 마이크 | speech |
| 2 | 사회자 마이크 | speech |
| 5–8 | 찬양 보컬 | vocal |
| 9–14 | 악기(기타/베이스/키보드/드럼) | inst |
| 15–16 | 반주(MR)/노트북 | playback |

---

## 구조

```
src/
  main/
    main.js        Electron 메인 · IPC · 윈도우
    osc-bus.js     단일 UDP 소켓 OSC 버스(node-osc Server 기반, 양방향)
    x32.js         X32 매니저(연결/채널 읽기/미터 구독/Scene 적용)
    x32-util.js    페이더·EQ 값 ↔ dB/Hz 변환 (순수 함수)
    feedback.js    피드백(하울링) 감지기
    scenes.js      교회용 Scene 템플릿 + 채널 맵
  preload/
    preload.js     contextBridge 안전 API
  renderer/
    index.html / styles.css / renderer.js   한국어 UI
test/
    mock-x32.js    가짜 X32(UDP) · unit/integration 테스트
```

### OSC 통신 메모

X32 는 요청을 보낸 **출발 포트로 응답**하므로 보내기/받기를 같은 소켓으로
처리해야 합니다. 이 앱은 `node-osc` 의 `Server` 가 자신의 바인딩 소켓으로
`send()` 할 수 있다는 점을 이용해 **소켓 하나로 양방향 통신**을 구현합니다
(`src/main/osc-bus.js`). 또한 페이더/EQ 같은 float 파라미터는 `node-osc` 가
`1.0` 을 정수로 인코딩하지 않도록 명시적 `{type:'f'}` 로 전송합니다.

---

## 믹서 확장 (어댑터 구조)

콘솔과 통신하는 부분은 **`MixerAdapter` 공통 인터페이스**(`src/main/mixer-base.js`)로
추상화되어 있습니다. UI·자동화 로직은 이 계약에만 의존하므로, 새 브랜드를 지원하려면
어댑터 하나만 추가하면 됩니다.

```
MixerAdapter (mixer-base.js)         ← 공통 계약 + capabilities
  └─ X32Manager (x32.js)             ← Behringer/Midas X32·M32 (구현 완료)
  └─ (XAirAdapter / WingAdapter …)   ← 추가 시 여기만 구현
```

- `src/main/mixer-registry.js` 에 한 줄 등록하면 **믹서 선택 화면에 자동으로 표시**됩니다.
  (`status: 'supported'` 는 바로 사용, `'planned'` 는 "추후 지원 예정"으로 비활성 표시)
- 각 어댑터는 `capabilities` 로 지원 기능을 선언해, 앱이 해당 믹서에서 가능한 기능만 켭니다.
- **아날로그 가이드**는 콘솔과 통신하지 않고 마이크로 분석하므로 **브랜드 무관**하게 동작합니다.

| 콘솔 | 상태 |
|------|------|
| Behringer/Midas **X32 / M32** | ✅ 지원 |
| Behringer **X-Air**, **Wing** / Yamaha / Allen&Heath / PreSonus | 🚧 추후 지원 예정 |

## 로드맵 (이후 단계)

- 실시간 페이더/미터 **양방향** 제어(앱에서 콘솔 페이더 직접 조작)
- 원격 페이지에서 예배 순서 큐·음소거까지 제어
- 콘솔 Scene/Snapshot 슬롯 연동, 변경 이력(로그)
- Scene 이름까지 다국어화

> 위 "주요 기능 + 추가 기능"(1~15)은 **모두 구현·테스트되어 있습니다.**

## 라이선스

MIT
